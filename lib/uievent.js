  ...
  uievent.handleMaterialFocus = function (ctrl) {
    setTimeout(function () {
      if (ctrl.nodeName === 'INPUT' || ctrl.nodeName === 'SELECT'
        ...
        var c = resources_1.container(ctrl);
...
function checkOnBlur(event, key, check, formatF) {
  ...
  setTimeout(function () {
    ...
    if (value.length > 0 && !check(value)) {
      var label = resources_1.getLabel(ctrl);
...
function emailOnBlur(event) {
  checkOnBlur(event, 'error_email', validation_util_1.isEmail);
}
exports.emailOnBlur = emailOnBlur;
function urlOnBlur(event) {
  checkOnBlur(event, 'error_url', validation_util_1.isUrl);
}
exports.urlOnBlur = urlOnBlur;
function phoneOnBlur(event) {
  checkOnBlur(event, 'error_phone', validation_util_1.tel.isPhone, formatter_1.formatter.removePhoneFormat);
}
exports.phoneOnBlur = phoneOnBlur;
function faxOnBlur(event) {
  checkOnBlur(event, 'error_fax', validation_util_1.tel.isFax, formatter_1.formatter.removeFaxFormat);
}
exports.faxOnBlur = faxOnBlur;
function ipv4OnBlur(event) {
  checkOnBlur(event, 'error_ipv4', validation_util_1.isIPv4);
}
exports.ipv4OnBlur = ipv4OnBlur;
function ipv6OnBlur(event) {
  checkOnBlur(event, 'error_ipv6', validation_util_1.isIPv6);
}
...
function currencyOnFocus(event, locale, currencyCode) {
  var ctrl = event.currentTarget;
  uievent.handleMaterialFocus(ctrl);
  if (ctrl.readOnly || ctrl.disabled || ctrl.value.length === 0) {
    return;
    ...
    if (c && resources_1.resources.currency && c.length > 0) {
      var currency = resources_1.resources.currency.getCurrency(c);
      if (currency) {
...
function formatCurrency(v, locale, currencyCode, includingCurrencySymbol) {
  return resources_1.resources.formatCurrency(v, currencyCode, locale, includingCurrencySymbol);
}
  ...
  if (numFormat !== null) {
    if (numFormat.indexOf('number') === 0) {
      ...
      if (strNums.length > 0 && isULong(strNums[1])) {
        ...
        return resources_1.resources.formatNumber(v, scale, locale);
      }
      else {
        return '' + v;
      }
      ...
      return resources_1.resources.format(v, numFormat, locale);
    }
  }
  else {
    return '' + v;
    ...
    if (form) {
      ...
      if (ctrl2) {
        ...
        if (smin2.length > 0 && !isNaN(smin2)) {
          ...
          if (n < min2) {
            var minLabel = resources_1.getLabel(ctrl2);
