...
function checkOnBlur(event, key, check, formatF) {
  ...
  setTimeout(function () {
    ...
    if (value.length > 0 && !check(value)) {
      ...
      var msg = uivalidator_1.formatText(r[key], label);
...
function phoneOnBlur(event) {
  checkOnBlur(event, "error_phone", validation_core_1.tel.isPhone, formatter_1.removePhoneFormat);
}
exports.phoneOnBlur = phoneOnBlur;
function faxOnBlur(event) {
  checkOnBlur(event, "error_fax", validation_core_1.tel.isFax, formatter_1.removeFaxFormat);
}
    ...
    if (value.length > 0) {
      ...
      if (pattern) {
        ...
        if (resource_key && !validation_core_1.isValidPattern(value, pattern, flags)) {
          var label = resources_1.resources.label(ctrl);
          var r = resources_1.resources.resource.resource();
          var msg = uivalidator_1.formatText(r[resource_key], label);
          uivalidator_1.addErrorMessage(ctrl, msg);
        }
      }
    }
...
function intOnFocus(event) {
  ...
  if (ctrl.readOnly || ctrl.disabled || ctrl.value.length === 0) {
    ...
    var v = removeSeparators(ctrl.value);
    if (!isNaN(v)) {
      ctrl.value = v;
    }
  }
}
exports.intOnFocus = intOnFocus;
...
function intOnBlur(event, groupSeparator) {
  ...
  setTimeout(function () {
    ...
    if (value.length > 0) {
      var value2 = removeSeparators(value);
      if (isNaN(value2)) {
        var r = resources_1.resources.resource.resource();
        var msg = uivalidator_1.formatText(r.error_number, label);
        uivalidator_1.addErrorMessage(ctrl, msg);
        return;
      }
      var n = parseFloat(value2);
      if (validateIntMinMax(ctrl, n, label)) {
        var r = formatInteger(n, groupSeparator);
        if (r !== ctrl.value) {
          ctrl.value = r;
        }
        uivalidator_1.removeError(ctrl);
...
function validateMinMax(ctrl, n, label, locale) {
  ...
  if (min !== null && min.length > 0) {
    min = parseFloat(min);
    if (n < min) {
      var msg = uivalidator_1.formatText(r.error_min, label, min);
      var maxd = ctrl.getAttribute("max");
      if (maxd && maxd.length > 0) {
        maxd = parseFloat(maxd);
        if (maxd === min) {
          msg = uivalidator_1.formatText(r.error_equal, label, maxd);
        }
      }
      uivalidator_1.addErrorMessage(ctrl, msg);
      return false;
  ...
  if (max !== null && max.length > 0) {
    max = parseFloat(max);
    if (n > max) {
      var msg = uivalidator_1.formatText(r.error_max, label, max);
      if (min && max === min) {
        msg = uivalidator_1.formatText(r.error_equal, label, max);
      }
      uivalidator_1.addErrorMessage(ctrl, msg);
      return false;
    }
      ...
      if (ctrl2) {
        ...
        if (smin2.length > 0 && !isNaN(smin2)) {
          ...
          if (n < min2) {
            ...
            var msg = uivalidator_1.formatText(r.error_min, label, minLabel);
            uivalidator_1.addErrorMessage(ctrl, msg);
            return false;
          }
        }
...
function validateIntMinMax(ctrl, n, label) {
  var min = ctrl.getAttribute("min");
  var r = resources_1.resources.resource.resource();
  if (min !== null && min.length > 0) {
    min = parseFloat(min);
  ...
  if (minField) {
    ...
    if (form) {
      var ctrl2 = ui_1.element(form, minField);
      if (ctrl2) {
        var smin2 = removeSeparators(ctrl2.value);
        if (smin2.length > 0 && !isNaN(smin2)) {
          var min2 = parseFloat(smin2);
          if (n < min2) {
            var minLabel = resources_1.resources.label(ctrl2);
