...
function format(v, format, locale) {
...
var formatter = (function () {
  ...
  formatter.formatCurrency = function (value, currencyCode, locale, includingCurrencySymbol) {
    ...
    if (resources_1.resources.currency) {
      currency = resources_1.resources.currency.currency(currencyCode);
    }
    if (!currency) {
      currency = usd;
    }
  ...
  formatter.formatNumber = function (value, scale, locale) {
    if (locale) {
  ...
  formatter.format = function (v, format, locale) {
    if (!format) {
      format = '#,###.00';
    }
