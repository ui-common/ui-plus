...
function checkRequired(ele, label) {
  ...
  if (required !== null && required !== "false") {
    if (value.length === 0) {
      ...
      var r = resources_1.resources.resource.resource();
      var s = r[errorKey];
      if (!s || s === "") {
        s = r.error_required;
      }
      var msg = formatText(s, label);
...
function checkMaxLength(ele, label) {
  ...
  if (maxlength && !isNaN(maxlength)) {
    ...
    if (value.length > imaxlength) {
      ...
      var msg = formatText(r.error_maxlength, label, maxlength);
      addErrorMessage(ele, msg);
      return true;
    }
  }
...
function checkMinLength(ele, label) {
  ...
  if (minlength !== null && !isNaN(minlength)) {
    ...
    if (value.length < iminlength) {
      ...
      var msg = formatText(r.error_minlength, label, minlength);
      addErrorMessage(ele, msg);
      return true;
    }
  }
...
function validateElement(ele, locale, includeReadOnly) {
  ...
  if (minlength !== null && !isNaN(minlength)) {
    var iminlength = parseInt(minlength, 10);
    if (value.length < iminlength) {
      var msg = formatText(r.error_minlength, l, minlength);
      addErrorMessage(ele, msg);
      return false;
    }
  }
  ...
  if (pattern) {
    ...
    if (!validation_core_1.isValidPattern(value, pattern, flags)) {
      ...
      if (resource_key) {
        var msg = formatText(r.resource_key, l);
        addErrorMessage(ele, msg);
      }
      else {
  ...
  if (datatype2 === "email") {
    if (value.length > 0 && !validation_core_1.isEmail(value)) {
      var msg = formatText(r.error_email, l);
      addErrorMessage(ele, msg);
      return false;
    }
  }
    ...
    if (isNaN(value)) {
      var msg = formatText(r.error_number, l);
      addErrorMessage(ele, msg);
      return false;
    }
    if (datatype2 === "integer" && !validation_core_1.isDigitOnly(value)) {
      var msg = formatText(r.error_number, l);
      addErrorMessage(ele, msg);
      return false;
    }
    ...
    if (smin !== null && smin.length > 0) {
      min = parseFloat(smin);
      if (n < min) {
        var msg = formatText(r.error_min, l, min);
    ...
    if (smax !== null && smax.length > 0) {
      ...
      if (n > max) {
        var msg = formatText(r.error_max, l, max);
        if (!min && max === min) {
      ...
      if (ctrl2) {
        ...
        if (smin2.length > 0 && !isNaN(smin2)) {
          ...
          if (n < min2) {
            ...
            var msg = formatText(r.error_min, l, minLabel);
            addErrorMessage(ele, msg);
            return false;
          }
        }
    ...
    if (!isNaN(v.getTime())) {
      ...
      if (smin && smin.length > 0) {
        if (smin === "now") {
          var d = new Date();
          if (v < d) {
            var msg = formatText(r.error_from_now, l);
            addErrorMessage(ele, msg);
            return false;
          }
        }
          ...
          if (v < d) {
            var msg = formatText(r.error_from_tomorrow, l);
            addErrorMessage(ele, msg);
            return false;
          }
        }
          ...
          if (!isNaN(d.getTime())) {
            if (v < d) {
              ...
              var msg = formatText(r.error_from, l, v2);
              addErrorMessage(ele, msg);
              return false;
            }
          }
      ...
      if (smax && smax.length > 0) {
        if (smax === "now") {
          var d = new Date();
          if (v > d) {
            var msg = formatText(r.error_after_now, l);
            addErrorMessage(ele, msg);
            return false;
          }
        }
          ...
          var d = addDays(trimTime(new Date()), 1);
          if (v > d) {
            var msg = formatText(r.error_after_tomorrow, l);
            addErrorMessage(ele, msg);
            return false;
          }
        }
          ...
          if (!isNaN(d.getTime())) {
            if (v > d) {
              var v2 = formatLongDateTime(d, "YYYY-MM-DD");
              var msg = formatText(r.error_after, l, v2);
              addErrorMessage(ele, msg);
              return false;
            }
          }
      ...
      if (afterField && ele.form) {
        ...
        if (ctrl2 && ctrl2.value.length > 0) {
          ...
          if (v <= mi_2) {
            var minLabel = resources_1.resources.label(ctrl2);
            var msg = formatText(r.error_after, l, minLabel);
            addErrorMessage(ele, msg);
            return false;
          }
        }
    ...
    if (!dt) {
      var msg = formatText(r.error_date, l);
      addErrorMessage(ele, msg);
      return false;
    }
    else {
      ...
      if (maxdate !== null || mindate !== null) {
        if (maxdate !== null) {
          ...
          if (dmaxdate && dt > dmaxdate) {
            var msg = formatText(r.error_max_date, l);
            addErrorMessage(ele, msg);
            return false;
          }
        }
        if (mindate !== null) {
          ...
          if (dmindate && dt < dmindate) {
            var msg = formatText(r.error_min_date, l);
            addErrorMessage(ele, msg);
            return false;
          }
        }
    ...
    if (!validation_core_1.isUrl(value)) {
      var msg = formatText(r.error_url, l);
      addErrorMessage(ele, msg);
      return false;
    }
  }
  else if (datatype2 === "phone") {
    var phoneStr = formatter_1.removePhoneFormat(value);
    if (!validation_core_1.tel.isPhone(phoneStr)) {
      var msg = formatText(r.error_phone, l);
      addErrorMessage(ele, msg);
      return false;
    }
  }
  else if (datatype2 === "fax") {
    var phoneStr = formatter_1.removeFaxFormat(value);
    if (!validation_core_1.tel.isFax(phoneStr)) {
      var msg = formatText(r.error_fax, l);
      addErrorMessage(ele, msg);
      return false;
    }
  }
    ...
    if (!validation_core_1.isValidCode(value)) {
      var msg = formatText(r.error_code, l);
      addErrorMessage(ele, msg);
      return false;
    }
  }
    ...
    if (!validation_core_1.isDashCode(value)) {
      var msg = formatText(r.error_dash_code, l);
      addErrorMessage(ele, msg);
      return false;
    }
  }
    ...
    if (!validation_core_1.isDigitOnly(value)) {
      var msg = formatText(r.error_digit, l);
      addErrorMessage(ele, msg);
      return false;
    }
  }
    ...
    if (!validation_core_1.isDashDigit(value)) {
      var msg = formatText(r.error_dash_digit, l);
      addErrorMessage(ele, msg);
      return false;
    }
  }
    ...
    if (!validation_core_1.isDashDigit(value)) {
      var msg = formatText(r.error_routing_number, l);
      addErrorMessage(ele, msg);
      return false;
    }
  }
    ...
    if (!validation_core_1.isCheckNumber(value)) {
      var msg = formatText(r.error_check_number, l);
      addErrorMessage(ele, msg);
      return false;
    }
  }
    ...
    if (countryCode) {
      ...
      if (countryCode === "US" || countryCode === "USA") {
        if (!validation_core_1.isUSPostalCode(value)) {
          var msg = formatText(r.error_us_post_code, l);
          addErrorMessage(ele, msg);
          return false;
        }
      }
        ...
        if (!validation_core_1.isCAPostalCode(value)) {
          var msg = formatText(r.error_ca_post_code, l);
          addErrorMessage(ele, msg);
          return false;
        }
      }
        ...
        if (!validation_core_1.isDashCode(value)) {
          var msg = formatText(r.error_post_code, l);
          addErrorMessage(ele, msg);
          return false;
        }
      }
    ...
    if (!validation_core_1.isIPv4(value)) {
      var msg = formatText(r.error_ipv4, l);
      addErrorMessage(ele, msg);
      return false;
    }
  }
    ...
    if (!validation_core_1.isIPv6(value)) {
      var msg = formatText(r.error_ipv6, l);
      addErrorMessage(ele, msg);
      return false;
    }
  }
