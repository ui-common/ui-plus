...
function checkRequired(ctrl, label) {
  ...
  if (required !== null && required !== 'false') {
    if (value.length === 0) {
      if (!label) {
        label = resources_1.getLabel(ctrl);
      }
...
function validateElement(ctrl, locale) {
  ...
  var parent = resources_1.container(ctrl);
  if (parent) {
  ...
  if (datatype2 === 'email') {
    ...
    if (datatype2 === 'currency' || datatype2 === 'string-currency') {
      ...
      if (currencyCode && resources_1.resources.currency && currencyCode.length > 0) {
        var currency = resources_1.resources.currency.getCurrency(currencyCode);
    ...
    var date = resources_1.resources.date.parse(value, dateFormat);
    if (!date) {
...
function addErrorMessage(ctrl, msg) {
  ...
  var parrent = resources_1.container(ctrl);
  if (parrent === null) {
    return;
  }
